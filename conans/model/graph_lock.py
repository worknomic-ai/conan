...
class _LockRequires:
    ...
    def remove(self, ref):
        """ remove requirements from lockfile. Matches by name, and if provided by version
        It will remove all matches, not only the first one
        """
        to_remove = []
        for k in self._requires:
            if k.name == ref.name and (not ref.version or k.version == ref.version):
                to_remove.append(k)
        for k in to_remove:
            self._requires.pop(k)
