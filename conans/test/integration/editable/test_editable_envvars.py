...
def test_editable_conf():
    ...
    dep = textwrap.dedent("""
        ...
        class Dep(ConanFile):
            def layout(self):
                self.folders.source = "mysource"
                self.folders.build = "mybuild"
                self.layouts.source.conf_info.append_path("user.myconf:myvar", "mylocalsrc")
                self.layouts.build.conf_info.append_path("user.myconf:myvar", "mylocalbuild")
                self.layouts.build.conf_info.update_path("user.mydictconf:mydict", {"a": "mypatha", "b": "mypathb"})
                self.layouts.build.conf_info.define_path("user.mydictconf2:mydict", {"c": "mypathc"})
        """)
    ...
    pkg = textwrap.dedent("""
        ...
        class Pkg(ConanFile):
            requires = "dep/1.0"
            def generate(self):
                conf = self.dependencies["dep"].conf_info.get("user.myconf:myvar")
                self.output.info(f"CONF: {conf}")
                dictconf = self.dependencies["dep"].conf_info.get("user.mydictconf:mydict", check_type=dict)
                self.output.info(f"CONFDICT: {dictconf}")
                dictconf2 = self.dependencies["dep"].conf_info.get("user.mydictconf2:mydict", check_type=dict)
