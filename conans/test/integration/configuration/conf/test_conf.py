...
def test_empty_conf_valid():
    ...
    class BasicConanfile(ConanFile):
        ...
        def generate(self):
            self.output.warning(f'My unset conf variable is: "{self.conf.get("user.conf:unset")}"')
            self.output.warning(f'My unset conf is {"NOT" if self.conf.get("user.conf:unset") == None else ""} set')
    """)
    ...
    tc.run("create . -c user.conf:unset=")
    assert 'pkg/1.0: WARN: My unset conf variable is: ""' in tc.out
    assert 'pkg/1.0: WARN: My unset conf is  set' in tc.out
    ...
    tc.run("create . -c user.conf:unset=Hello")
