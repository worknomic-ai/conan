...
class ConanOutput:
    ...
    @classmethod
    def define_warnings_as_errors(cls, warnings_as_errors):
        cls._warnings_as_errors = warnings_as_errors or []
    ...
    def warning(self, msg, warn_tag=None):
        if self._conan_output_level <= LEVEL_WARNING:
            ...
            tag = warn_tag if warn_tag is not None else "unknown"
            if "all" in self._warnings_as_errors or tag in self._warnings_as_errors:
                raise ConanException(f"{warn_tag_msg}{msg}")
