...
class CMakeDeps(object):
    ...
    def content(self):
        ...
        for require, dep in list(host_req.items()) + list(build_req.items()) + list(test_req.items()):
            ...
            suffix = self.build_context_suffix.get(dep.ref.name, "") if require.build else ""
            if cmake_find_mode in (FIND_MODE_CONFIG, FIND_MODE_BOTH):
                pkg_name = self.get_cmake_package_name(dep, module_mode=False) + suffix
                conandeps_content.append(f"find_package({pkg_name} CONFIG REQUIRED)")
            elif cmake_find_mode == FIND_MODE_MODULE:
                pkg_name = self.get_cmake_package_name(dep, module_mode=True) + suffix
                conandeps_content.append(f"find_package({pkg_name} MODULE REQUIRED)")

        # remove duplicates but preserve order
        conandeps_content = list(dict.fromkeys(conandeps_content))
        conandeps_content.insert(0, "message(STATUS \"Conan: Using autogenerated conandeps.cmake\")")
        ret["conandeps.cmake"] = "\n".join(conandeps_content) + "\n"
