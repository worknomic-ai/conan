...
from conans.client.graph.graph import BINARY_BUILD, BINARY_CACHE, BINARY_DOWNLOAD, BINARY_EDITABLE, \
    BINARY_SYSTEM_TOOL, BINARY_UPDATE, BINARY_EDITABLE_BUILD, BINARY_SKIP, BINARY_PLATFORM
...
class BinaryInstaller:
    ...
    def _install_source(self, node, remotes):
        ...
        conanfile = node.conanfile
        if node.binary in (BINARY_EDITABLE, BINARY_PLATFORM, BINARY_SYSTEM_TOOL):
            return
    ...
    def _handle_package(self, package, install_reference, handled_count, total_count):
        if package.binary in (BINARY_SYSTEM_TOOL, BINARY_PLATFORM):
            return
